...
exports.getCandidateAnalytics = void 0;
...
const getCandidateAnalytics = async (userId, range) => {
    ...
    const allTimeAttempts = await prisma_1.default.attempt.findMany({
        where: { userId, completed: true },
        select: { submittedAt: true, score: true, percentage: true },
        orderBy: { submittedAt: "desc" },
    });
    // ── Overall Rank/Percentile ────────────────────────────────────────────────
    const allUserAverages = await prisma_1.default.attempt.groupBy({
        by: ["userId"],
        _avg: { percentage: true },
    });
    const myAvg = attempts.length > 0
        ? attempts.reduce((sum, a) => sum + a.percentage, 0) / attempts.length
        : 0;
    const totalUsers = allUserAverages.length || 1;
    const countLower = allUserAverages.filter((u) => (u._avg.percentage || 0) <= myAvg).length;
    const percentile = (countLower / totalUsers) * 100;
    // ── Streaks ────────────────────────────────────────────────────────────────
    const uniqueDates = Array.from(new Set(allTimeAttempts.map((a) => a.submittedAt.toISOString().split("T")[0]))).sort((a, b) => b.localeCompare(a)); // Descending order
    let currentDaily = 0;
    let longestDaily = 0;
    let weekly = 0;
    if (uniqueDates.length > 0) {
        ...
        const uniqueWeeks = Array.from(new Set(allTimeAttempts.map((a) => `${a.submittedAt.getFullYear()}-W${getWeekNumber(a.submittedAt)}`)));
    ...
    for (const a of attempts) {
        ...
        if (a.percentage > highestScore)
            highestScore = a.percentage;
        if (a.percentage < lowestScore)
            lowestScore = a.percentage;
    ...
    let goal = await prisma_1.default.userGoal.findUnique({
        where: { userId },
    });
    if (!goal) {
        goal = await prisma_1.default.userGoal.create({
            data: { userId },
        });
    }
    ...
    const weeklyTestsCompleted = await prisma_1.default.attempt.count({
        where: { userId, completed: true, submittedAt: { gte: sevenDaysAgo } },
    });
    const monthlyTestsCompleted = await prisma_1.default.attempt.count({
        where: { userId, completed: true, submittedAt: { gte: thirtyDaysAgo } },
    });
    const weeklyAnswers = await prisma_1.default.answer.count({
        where: {
            attempt: {
                userId,
                completed: true,
    ...
    let highestAccuracySubject = null;
    let lowestAccuracySubject = null;
    let fastestSubject = null;
    let slowestSubject = null;
    let mostImprovedSubject = null;
    let highestSkipRateSubject = null;
    let highestIncorrectRateSubject = null;
    let mostPracticedSubject = null;
    let leastPracticedSubject = null;
    const statements = [];
...
exports.getCandidateAnalytics = getCandidateAnalytics;
