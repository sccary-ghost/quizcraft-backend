...
exports.QuestionOsService = void 0;
const prisma_1 = __importDefault(require("../utils/prisma"));
class QuestionOsService {
    /**
    ...
    static async queryQuestions(params) {
        ...
        const whereClause = { isDeleted: false, isBank: true };
     ...
     * Property Management
     */
    static async getProperties(organizationId) {
        return await prisma_1.default.questionProperty.findMany({
            where: organizationId ? { organizationId } : {},
            orderBy: { displayOrder: 'asc' }
        });
    }
    static async createProperty(data) {
        return await prisma_1.default.questionProperty.create({
            data: {
                name: data.name,
                slug: data.slug,
                type: data.type,
     ...
     * Bulk Operations
     */
    static async bulkUpdate(questionIds, updates) {
     ...
     * Views Management
     */
    static async getViews(userId) {
        return await prisma_1.default.questionView.findMany({
            where: {
                OR: [
                    { createdBy: userId },
                    { isShared: true }
    ...
    static async getCollections() {
        return await prisma_1.default.questionCollection.findMany({
            orderBy: { name: 'asc' },
            include: {
                _count: {
                    select: { items: true }
    ...
    // Helper
    static getValueColumnForType(type) {
        switch (type) {
            case 'NUMBER':
            case 'RATING':
                return 'numberValue';
...
exports.QuestionOsService = QuestionOsService;
