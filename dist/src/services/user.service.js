 ...
 */
const getCandidatesList = async (search = "", statusFilter = "", sortBy = "name") => {
...
exports.getCandidatesList = getCandidatesList;
/**
 * Get profile metrics for a specific candidate.
 */
const getCandidateProfileDetails = async (userId) => {
    ...
    const completedAttempts = await prisma_1.default.attempt.findMany({
    ...
    const ongoingCount = await prisma_1.default.attempt.count({
        where: { userId, completed: false },
    });
    const totalTests = await prisma_1.default.attempt.count({
        where: { userId },
    });
    ...
    const totalCorrectAnswers = completedAttempts.reduce((sum, att) => sum + att.answers.filter((ans) => ans.isCorrect).length, 0);
    const totalQuestionsAnswered = completedAttempts.reduce((sum, att) => sum + att.answers.length, 0);
    const overallAccuracy = totalQuestionsAnswered > 0
        ? (totalCorrectAnswers / totalQuestionsAnswered) * 100
        : 0;
    // Time metrics
    const totalTimeSpent = completedAttempts.reduce((sum, att) => sum + att.answers.reduce((s, ans) => s + ans.timeSpent, 0), 0);
...
exports.getCandidateProfileDetails = getCandidateProfileDetails;
/**
 * Get all attempts history for a candidate.
 */
const getCandidateAttemptsHistory = async (userId) => {
...
exports.getCandidateAttemptsHistory = getCandidateAttemptsHistory;
/**
 * Edit candidate basic details and active status.
 */
const updateCandidateDetails = async (userId, data) => {
    ...
    if (data.mobileNumber) {
        const existingMobile = await prisma_1.default.user.findFirst({
