...
import { startQuizAttempt, submitQuiz } from "../services/quiz.service";
import { handleEmailOtpCooldown } from "../services/otp.service";
...
describe("Concurrency and Integrity Tests", () => {
  ...
  it("Test 1: 20 concurrent POST /quiz/start (Only 1 success)", async () => {
    const promises = Array.from({ length: 20 }).map(() => startQuizAttempt(userId, quizId));
  ...
  it("Test 2: 20 concurrent submitQuiz (Only 1 submission committed)", async () => {
    const promises = Array.from({ length: 20 }).map(() => submitQuiz(userId, quizId, [0]));
  ...
  it("Test 3: 20 concurrent requestEmailOtp (cooldown enforced)", async () => {
    ...
    const promises = Array.from({ length: 20 }).map(() => 
      handleEmailOtpCooldown(testEmail, "hashtest", new Date(Date.now() + 600000))
    );
    ...
    await handleEmailOtpCooldown(testEmail, "hashtest", new Date(Date.now() + 600000));
  ...
  it("Test 4: Force transaction failure midway (rollback check)", async () => {
    ...
    const promises = Array.from({ length: 5 }).map(() => startQuizAttempt(userId, "invalid_id"));
    const results = await Promise.allSettled(promises);
  ...
  it("Test 5: Expired OTP cleanup / reset", async () => {
    ...
    // Request new OTP
    await handleEmailOtpCooldown(testEmail, "newhash", new Date(Date.now() + 600000));
