...
function removeQuestionNumber(line: string) {
  return line
    .replace(/^Q\.?\s*\d+[.)]?\s*/i, "")
    .replace(/^\d+[.)]\s*/, "")
    .replace(/^\.\s*/, "")
  ...
  for (const line of lines) {
    if (isQuestionStart(line)) {
      if (
        ...
        if (tempAnswerLabel === "a") current.correctAnswer = current.optionA;
        else if (tempAnswerLabel === "b") current.correctAnswer = current.optionB;
        else if (tempAnswerLabel === "c") current.correctAnswer = current.optionC;
        else if (tempAnswerLabel === "d") current.correctAnswer = current.optionD;
    ...
    if (looksLikeOption) {
      ...
      if (optionCount === 1) {
        ...
        currentOption = "A";
        if (hasAsterisk) tempAnswerLabel = "a";
      } else if (optionCount === 2) {
        current.optionB = cleaned;
        currentOption = "B";
        if (hasAsterisk) tempAnswerLabel = "b";
      } else if (optionCount === 3) {
        current.optionC = cleaned;
        currentOption = "C";
        if (hasAsterisk) tempAnswerLabel = "c";
      } else if (optionCount === 4) {
        current.optionD = cleaned;
        currentOption = "D";
        if (hasAsterisk) tempAnswerLabel = "d";
      }
